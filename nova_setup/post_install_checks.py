...
def perform_post_install_checks(check_gnome_specific_tools: bool = False):
    ...
    for tool_name_check in tools_to_verify:
        ...
        # Tentativo aggiuntivo per i tool Cargo/scripted se non trovati con il solo nome,
        # controllando percorsi specifici.
        explicit_path_found = False
        actual_cmd_for_version = [tool_name_check] # Default a usare il nome del tool
        ...
        if not tool_exists and user_to_run_check_as == shared_state.TARGET_USER:
            ...
            if tool_name_check == "zoxide" or tool_name_check == "atuin": # Esempio per scripted tools
            ...
            for p_path in possible_paths:
                if command_utils.check_command_exists([str(p_path)], as_user=user_to_run_check_as):
                    tool_exists = True
                    explicit_path_found = True
                    actual_cmd_for_version = [str(p_path)] # Usa il percorso completo per il version check
        ...
        if tool_exists:
            ...
            try:
                for v_flag_opt in ["--version", "-V", "version"]:
                    ...
                    if tool_name_check == "pip3" and v_flag_opt == "--version": # pip3 usa 'show pip'
                        cmd_list_ver = actual_cmd_for_version[:1] + ["show", "pip"] # Usa il nome base per pip show
