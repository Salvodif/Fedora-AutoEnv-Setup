...
from scripts.myrich import (
    print_header, print_info, print_error, print_success, print_step, console
)
...
# Add any system packages you commonly use.
# Example: "util-linux-user" for chsh, "zsh" for Zsh shell, etc.
DNF_PACKAGES_TO_INSTALL = [
    ...
    # "util-linux-user", # Example: for chsh command
]

# Dictionary of Flatpak applications to install { "app-id": "Friendly Name" }
# Find app-ids on Flathub: https://flathub.org/
...
def _install_dnf_packages() -> bool:
    ...
    if not DNF_PACKAGES_TO_INSTALL:
        print_info("No DNF packages specified for installation.")
        return True
...
def _install_flatpak_apps() -> bool:
    ...
    if not FLATPAK_APPS_TO_INSTALL:
        print_info("No Flatpak applications specified for installation.")
        return True
    ...
    if not flathub_exists:
        ...
        # run_command with check=True (default) returns bool for sudo commands
...
def run_additional_packages_installation():
    ...
    dnf_success = _install_dnf_packages()
    console.rule() # Visual separator

    # Group Flatpak installations
    flatpak_success = _install_flatpak_apps()
