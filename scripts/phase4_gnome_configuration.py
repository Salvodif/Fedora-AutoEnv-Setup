...
from scripts import system_utils # Import system_utils

# --- Constants ---
GEXT_CLI_MODULE = "gnome_extensions_cli.cli" 

# --- Helper Functions ---
# _get_target_user, _get_user_home, _check_or_install_gext_cli,
# _install_dnf_packages_ph4, _install_ego_extension, _install_git_extension
# (Keep these existing helper functions as they are)
...
# REMOVE _install_flatpak_apps_ph4 as it's now in system_utils.py
...
def run_phase4(app_config: dict) -> bool:
    ...
    if flatpak_apps_to_install:
        # Use the new system_utils function
        if not system_utils.install_flatpak_apps(
            apps_to_install=flatpak_apps_to_install,
            system_wide=True, # Explicitly system_wide
            print_fn_info=con.print_info,
            print_fn_error=con.print_error,
            print_fn_sub_step=con.print_sub_step
        ):
            overall_success = False
            # system_utils.install_flatpak_apps will print its own error summary.
            # We can add a phase-specific note if needed.
